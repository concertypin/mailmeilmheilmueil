...
test("filters the inbox by every AI metadata field", async () => {
    ...
    await screen.getByRole("button", { name: /검토함/ }).click();
    await screen.getByRole("button", { name: /발송 대기/ }).click();
    await screen.getByRole("button", { name: /AI 분석 결과/ }).click();
    await expect.element(screen.getByText("문자열 검색")).toBeVisible();
    await expect.element(screen.getByText("검색")).toBeVisible();
    await expect.element(screen.getByText("필터 초기화")).toBeVisible();
    await screen.getByRole("button", { name: /중요 메일/ }).click();
    await expect.element(screen.getByText("중요 메일")).toBeVisible();
    await screen.getByRole("button", { name: /카테고리/ }).click();
    await expect
        .element(screen.getByRole("option", { name: "전체" }))
        .toBeVisible();
    await expect
        .element(screen.getByRole("option", { name: "대외활동" }))
        .toBeVisible();
    await expect
        .element(screen.getByRole("option", { name: "취업정보" }))
        .toBeVisible();
    await expect
        .element(screen.getByRole("option", { name: "장학/학사" }))
        .toBeVisible();
    await expect
        .element(screen.getByRole("option", { name: "비교과" }))
        .toBeVisible();
    await expect
        .element(screen.getByRole("option", { name: "학술/행사" }))
        .toBeVisible();
    await expect
        .element(screen.getByRole("option", { name: "직업훈련" }))
        .toBeVisible();
});

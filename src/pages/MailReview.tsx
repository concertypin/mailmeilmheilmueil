...
export default function MailReview() {
    ...
    const { get, review, forceAnalysis } = useMailData();
    const isReviewMode = _searchParams.get("mode") === "review";
    const [isForceAnalyzing, setIsForceAnalyzing] = useState(false);
    const [forceAnalysisError, setForceAnalysisError] = useState<string | null>(
        null
    );
    ...
    async function handleForceAnalysis(): Promise<void> {
        if (!item) return;
        setIsForceAnalyzing(true);
        setForceAnalysisError(null);
        try {
            const result = await forceAnalysis(item);
            setItem(result);
        } catch (err: unknown) {
            setForceAnalysisError(
                err instanceof Error ? err.message : String(err)
            );
        } finally {
            ...
            {item && !isLoading ? (
                isReviewMode ? (
                    <MailReviewPanel
                        ...
                        isForceAnalyzing={isForceAnalyzing}
                        forceAnalysisError={forceAnalysisError}
                    />
                ) : (

import { SMTPServer, type SMTPServerDataStream, type SMTPServerEnvelope } from "smtp-server";
import PostalMime, { type Address, type Email } from "postal-mime";
import { Timestamp } from "firebase-admin/firestore";
import { queuedMail, firestoreRepository, type MailRepository } from "./repository";
...
class SmtpDataError extends Error {
    constructor(message: string, readonly responseCode: number) {
        super(message);
...
function mailboxes(addresses: Address[] | undefined): string[] {
    return (addresses ?? []).flatMap((address) => "address" in address && address.address ? [address.address] : []);
}

function sender(email: Email, envelope: SMTPServerEnvelope): { name: string; address: string } {
    if (email.from && "address" in email.from && email.from.address) {
        return { name: email.from.name, address: email.from.address };
    }
    return { name: "", address: envelope.mailFrom === false ? "" : envelope.mailFrom.address };
}

export async function parseMailMessage(raw: Buffer, envelope: SMTPServerEnvelope, receivedAt = Timestamp.now()): Promise<Omit<MailItem, "id">> {
    let email: Email;
    try {
    ...
    const recipients = [...mailboxes(email.to), ...mailboxes(email.cc), ...envelope.rcptTo.map((recipient) => recipient.address)];
    return queuedMail({
...
export async function persistParsedMail(raw: Buffer, envelope: SMTPServerEnvelope, repository: MailRepository): Promise<string> {
...
async function handleData(
    ...
    try {
        ...
        void processMailItem(id, repository).catch((error: unknown) => process.stderr.write(`Mail processing failed: ${String(error)}\n`));
    } catch (error: unknown) {
        if (error instanceof SmtpDataError) {
            callback(error);
        } else {
            process.stderr.write(`SMTP message handling failed: ${String(error)}\n`);
...
export function startTestSmtpServer(
    ...
    server.listen(port, "127.0.0.1", () => process.stdout.write(`SMTP test server listening on 127.0.0.1:${port}\n`));
    return server;
}

...
export function createRoutes(repository: MailRepository = firestoreRepository): Hono {
    const app = new Hono();
    ...
    app.post("/api/mails/:id/review", async (context) => {
        ...
        return context.json({ id, status: "reviewed", reviewedAt: reviewedAt.toDate().toISOString() });
    });
